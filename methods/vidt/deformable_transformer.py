...
class DeformableTransformer(nn.Module):
    ...
    def __init__(self, d_model=256, nhead=8, use_encoder=False, num_encoder_layers=6, num_decoder_layers=6, 
                 ...
                 without_decoder=False, single_branch=False, interleave=False, det_token_num=100, inter_token_num=100, use_bg=False):
        super().__init__()
    ...
    def forward(self, srcs, masks, pos_embeds):
        ...
        if not self.interleave:
            ...
            reference_points = self.decoder.bbox_embed[-1](memory).sigmoid()
            sub_reference_points = self.decoder.sub_bbox_embed[-1](memory).sigmoid()
            inter_reference_points = self.decoder.inter_bbox_embed[-1](memory).sigmoid()
            inter_vector_points = self.decoder.inter_vector_embed[-1](memory).sigmoid()
            class_enc = self.decoder.class_embed[-1](memory)
            verb_enc = self.decoder.verb_embed[-1](memory)
            enc_outputs_class = class_enc.sigmoid() * verb_enc.sigmoid()

            _, topk_ind = torch.topk(enc_outputs_class.max(-1)[0], self.det_token_num, axis=1)
            topk_ind = topk_ind // enc_outputs_class.shape[2]

            reference_points = torch.gather(reference_points, 1, 
                               topk_ind.unsqueeze(-1).repeat(1,1,reference_points.shape[-1]))
            sub_reference_points = torch.gather(sub_reference_points, 1, 
                               topk_ind.unsqueeze(-1).repeat(1,1,sub_reference_points.shape[-1]))
            ...
            inter_vector_points = torch.gather(inter_vector_points, 1, 
                               topk_ind.unsqueeze(-1).repeat(1,1,inter_vector_points.shape[-1]))
            class_enc = torch.gather(class_enc, 1, 
                               topk_ind.unsqueeze(-1).repeat(1,1,class_enc.shape[-1]))    
            verb_enc = torch.gather(verb_enc, 1, 
                               topk_ind.unsqueeze(-1).repeat(1,1,verb_enc.shape[-1]))
            tgt = torch.gather(memory, 1, 
                               topk_ind.unsqueeze(-1).repeat(1,1,memory.shape[-1]))
            inter_tgt = tgt
            ...
            # Return if no decoder
            if self.without_decoder:
                return (init_reference_out, init_sub_reference_out, 
