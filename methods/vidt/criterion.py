...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_interactions, log=True):
        """
        Purpose: Computes the classification loss (Negative Log-Likelihood) for object labels. 
        Targets are expected to contain the key "labels" with a tensor of dimension [number of target boxes].
        """

        # pred_logits ( predicted class logits) should exists in the 'outputs'
    ...
    def loss_verb_labels(self, outputs, targets, indices, num_interactions):
        """
        Purpose: Compute classification loss for verb labels
        Targets are expected to contain the key "verb_labels" with a tensor of appropriate dimensions.
        """
    ...
    def _neg_loss(self, pred, gt):
        ''' 
        Purpose: Modified version of the Focal Loss calculation.
        Remarks: Modified focal loss. Exactly the same as CornerNet. Runs faster and costs a little bit more memory
        '''
    ...
    def loss_cardinality(self, outputs, targets, indices, num_boxes):
        ...
        This is not really a loss, it is intended for logging purposes only. It doesn't propagate gradients.add()
        
        Cardinality Loss: Absolute error between the predicted NUMBER of objects and the actual (ground truth) number of objects.
        """
    ...
    def loss_interaction_vector(self, outputs, targets, indices, num_interactions):
        """
        Purpose: Computes losses related to interaction vectors (if used).
        """
        # TODO: test
        # raise NotImplementedError("Implement interaction vector loss")
        assert "pred_interaction_vectors" in outputs
        # assert "verb_vecs" in targets
        
        # vector_preds = outputs['pred_interaction_vectors']
        # vector_gts = targets['verb_vecs']
    ...
    def loss_interaction_boxes(self, outputs, targets, indices, num_interactions):
        # TODO: test
        # raise NotImplementedError("Implement interaction vector loss")
        assert "pred_interaction_boxes" in outputs
        
        idx = self._get_src_permutation_idx(indices)
        
        pred_boxes = outputs['pred_interaction_boxes'][idx]
        target_boxes = torch.cat([t['verb_boxes'][i] for t, (_, i) in zip(targets, indices)], dim=0)
        
        losses = {}
        loss_inter_bbox = F.l1_loss(pred_boxes, target_boxes, reduction='none')
        losses['loss_verb_bbox'] = loss_inter_bbox.sum() / num_interactions

        loss_verb_giou = 1 - torch.diag(generalized_box_iou(box_cxcywh_to_xyxy(pred_boxes),
                                                            box_cxcywh_to_xyxy(target_boxes)))

        losses['loss_verb_giou'] = loss_verb_giou.sum() / num_interactions
    ...
    def get_loss(self, loss, outputs, targets, indices, num, **kwargs):
        loss_map = {
            ...
            'interaction_boxes': self.loss_interaction_boxes
        }
