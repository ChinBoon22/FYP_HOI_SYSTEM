...
class ReconfiguredAttentionModule(nn.Module):
    ...
    def forward(self, x, det, inter, mask=None, cross_attn=False, inter_det_cross_attn = False, 
                ...
                inter_patch_cross_attn_mask=None, bg=None):
        """ Forward function.
        ...
        if not cross_attn and not inter_patch_cross_attn:
            ...
            x = x.view(B, N, C)
            if bg:
                x = torch.cat([x, det, inter, bg], dim=1)
            else:
                x = torch.cat([x, det, inter], dim=1) # t.s. [B, H*W + 100 + 100, 48]
            full_qkv = self.qkv(x) # t.s. [B, H*W + 100 + 100, 144]
            if bg:
                patch_qkv, det_qkv, inter_qkv, bg_qkv = full_qkv[:, :N, :], \
                                                        full_qkv[:, N:N+self.det_token_num, :], \
                                                        full_qkv[:, N+self.det_token_num:N+self.det_token_num+self.inter_token_num, :], \
                                                        full_qkv[:, N+self.det_token_num+self.inter_token_num : N+self.det_token_num+self.inter_token_num+self.bg_token_num , :]
            else:
            ...
            if bg:
                x = torch.cat([shifted_x, cross_x, det, inter, bg], dim=1)
            else:
                x = torch.cat([shifted_x, cross_x, det, inter], dim=1)
            full_qkv = self.qkv(x)
            if bg:
                
                patch_qkv, cross_patch_qkv, det_qkv, inter_qkv, bg_qkv = \
                    full_qkv[:, :N, :], full_qkv[:, N:N + ori_N, :], full_qkv[:, N + ori_N:N+ori_N+self.det_token_num, :], \
                        full_qkv[:,N+ori_N+self.det_token_num:N+ori_N+self.det_token_num+self.inter_token_num, :], \
                            full_qkv[:,N+ori_N+self.det_token_num+self.inter_token_num:N+ori_N+self.det_token_num+self.inter_token_num+self.bg_token_num, :]
        ...
        # [BG] tokens
        if bg:
            bg_qkv = bg_qkv.view(B, -1, 3, self.num_heads, C // self.num_heads)
            bg_qkv = bg_qkv.permute(2, 0, 3, 1, 4)
            bg_q, bg_k, bg_v = bg_qkv[0], bg_qkv[1], bg_qkv[2] # each has shape: [B, 3, 100, 16]
            
            bg_q = bg_q * self.scale
            # [B, 8, 100, C] matmul [B,8,C, 100+100] = [B, 8,100, 200]
            bg_attn = (bg_q @ bg_k.transpose(-2, -1)) # shape: [B, num_heads, 100, 100]
            # apply cross-attention mask if available
            # if bg_patch_cross_attn_mask is not None:
            #     bg_attn = bg_attn + bg_patch_cross_attn_mask
            bg_attn = self.softmax(bg_attn)
            bg_attn = self.attn_drop(bg_attn)
            # [B, 3, 100, 200] matmul [B, 3, 200, C] = [B, 3, 100, C]
            bg_x = (bg_attn @ bg_v).transpose(1, 2).reshape(B, -1, C) # [B, 100, 48]
        ...
        if bg:
            x = torch.cat([patch_x.view(B, H*W, C), det_x, inter_x, bg_x], dim=1) # shape: [B, H*W + 100 + 100 + 100, 48]
        else:
        ...
        inter_x = x[:, H * W + self.det_token_num:H * W + self.det_token_num + self.inter_token_num, :]
        if bg:
            bg_x = x[:, H * W + self.det_token_num + self.inter_token_num : H * W + self.det_token_num + self.inter_token_num + self.bg_token_num , :]
        else:
            bg_x = None

        return patch_x, det_x, inter_x, bg_x
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, mask_matrix, pos, cross_attn, inter_cross_attn, cross_attn_mask, 
                inter_patch_cross_attn=False, inter_patch_cross_attn_mask=None, with_bg_tokens=False):
        """ Forward function.
        ...
        if with_bg_tokens: # with BG tokens
            assert L == H * W + self.det_token_num + self.inter_token_num + self.bg_token_num, "input feature has wrong size"
        else:
        ...
        if with_bg_tokens:
            x, det, inter, bg = x[:, :H * W, :], \
                                x[:, H * W:(H*W)+self.det_token_num, :], \
                                x[:, (H*W)+self.det_token_num:(H*W)+self.det_token_num+self.inter_token_num, :], \
                                x[:, (H*W)+self.det_token_num+self.inter_token_num : (H*W)+self.det_token_num+self.inter_token_num+self.bg_token_num , :]
        else:
            x, det, inter = x[:, :H * W, :], x[:, H * W:(H*W)+self.det_token_num, :], x[:, (H*W)+self.det_token_num:(H*W)+self.det_token_num+self.inter_token_num, :]
        ...
        if with_bg_tokens:
            patch_pos, det_pos, inter_pos, bg_pos = pos
        else:
        ...
        if with_bg_tokens:
            bg_pos = self.bg_pos_linear(bg_pos)
        ...
        # W-MSA/SW-MSA
        shifted_x, det, inter, bg = self.attn(shifted_x, mask=attn_mask,
                                   ...
                                   bg=bg if with_bg_tokens else None)
...
class PatchMerging(nn.Module):
    ...
    def forward(self, x, H, W, with_bg_tokens=False):
        """ Forward function.
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, H, W, det_pos, inter_pos, input_mask, 
                ...
                inter_patch_cross_attn=False, bg_pos=None):
        """ Forward function.
        ...
        if bg_pos is not None:
            pos = (patch_pos, det_pos, inter_pos, bg_pos)
        else:
            pos = (patch_pos, det_pos, inter_pos)
        ...
        for n_blk, blk in enumerate(self.blocks):
            ...
            if self.use_checkpoint:
                ...
                x = blk(x, attn_mask,
                        ...
                        inter_patch_cross_attn_mask=inter_patch_cross_attn_mask,
                        with_bg_tokens=bool(bg_pos)) # shape: [B, H*W, 48]
...
class SwinTransformer(nn.Module):
    ...
    def finetune_det(self, method, det_token_num=100, inter_token_num=100, pos_dim=256, 
        ...
        # BG tokens
        if bg_token_num:
            self.bg_token_num = bg_token_num
            self.bg_token = nn.Parameter(torch.zeros(1, bg_token_num, self.num_features[0]))
            self.bg_token = trunc_normal_(self.bg_token, std=.02)
        else:
            self.bg_token = None
        ...
        if bg_token_num:
            bg_pos_embed = torch.zeros(1, bg_token_num, pos_dim)
            bg_pos_embed = trunc_normal_(bg_pos_embed, std=.02)
            self.bg_pos_embed = torch.nn.Parameter(bg_pos_embed)
        ...
        for layer in self.layers:
            ...
            for block in layer.blocks:
                ...
                if bg_token_num:
                    block.bg_token_num = bg_token_num
                    block.bg_pos_linear = nn.Linear(pos_dim, block.dim)
                    block.attn.bg_token_num = bg_token_num
    ...
    def forward(self, x, mask):
        ...
        if self.bg_token is not None:
            bg_token = self.bg_token.expand(B, -1, -1) # shape: [B, 100, 48]
            bg_pos = self.bg_pos_embed
        ...
        for stage in range(self.num_layers):
            ...
            if self.bg_token is not None:
                x = torch.cat([x, bg_token], dim=1) # shape: [B, H/4*W/4 + 100 + 100 + 100, 48]
            ...
            x_out, H, W, x, Wh, Ww = layer(x, Wh, Ww,
                                        ...
                                        #    input_mask=mask,
                                           ...
                                           cross_attn=cross_attn, inter_cross_attn=inter_cross_attn,
                                           inter_patch_cross_attn=inter_patch_cross_attn,
                                           bg_pos = bg_pos if self.bg_token else None)
            if self.bg_token:
                # FIXME: change to use H, W or Wh, Ww
                x, det_token, inter_token, bg_token = x[:, :-(self.det_token_num + self.inter_token_num + self.bg_token_num), :], \
                                            x[:, -(self.det_token_num + self.inter_token_num + self.bg_token_num):-(self.inter_token_num + self.bg_token), :], \
                                            x[:, -(self.inter_token_num + self.bg_token):-self.bg_token, :], \
                                            x[:, -self.bg_token:, :]
            else:    
            ...
            if stage > 0:
                if self.bg_token:
                    patch_out = x_out[:, :-(self.det_token_num + self.inter_token_num + self.bg_token_num), :].view(B, H, W, -1).permute(0, 3, 1, 2)
                else:
        ...
        # final det token
        if self.bg_token:
            det_tgt = x_out[:, -(self.det_token_num + self.inter_token_num + self.bg_token_num):-(self.inter_token_num + self.bg_token_num), :].permute(0, 2, 1)
        else:
            det_tgt = x_out[:, -(self.det_token_num + self.inter_token_num):-self.inter_token_num, :].permute(0, 2, 1)
        
        # final inter token
        if self.bg_token:
            inter_tgt = x_out[:, -(self.inter_token_num + self.bg_token):-self.bg_token, :].permute(0, 2, 1) # shape: [1, 256, 100]
        else:
            inter_tgt = x_out[:, -self.inter_token_num:, :].permute(0, 2, 1) # shape: [1, 256, 100]
            
        if self.bg_token:
            bg_final = x_out[:, -self.bg_token:, :].permute(0, 2, 1)
        else:
            bg_final = None
        ...
        if self.bg_token:
            bg_pos = bg_pos.permute(0, 2, 1)
        else:
            bg_pos = None

        return patch_outs, det_tgt, inter_tgt, det_pos, inter_pos, bg_final, bg_pos
