...
class Stage2Assigner(nn.Module):
    def __init__(self, num_queries, positive_fraction, threshold, max_k=4):
        super().__init__()
    ...
    def forward(self, outputs, targets, return_cost_matrix=False):
        ...
        for b in range(bs):
            # Object
            obj_iou, _ = box_iou(
                  box_cxcywh_to_xyxy(targets[b]['obj_boxes']),
                  box_cxcywh_to_xyxy(outputs['pred_boxes'][b]),
            )

            # Interaction
            verb_iou, _ = box_iou(
                  box_cxcywh_to_xyxy(targets[b]['verb_boxes']),
                  box_cxcywh_to_xyxy(outputs['pred_interaction_boxes'][b]),
            )
            
            matched_idxs, matched_labels = self.proposal_matcher(obj_iou)  # proposal_id -> highest_iou_gt_id, proposal_id -> [1 if iou > 0.6, 0 ow]
            sampled_idxs, sampled_gt_classes = self._sample_proposals(  # list of sampled proposal_ids, sampled_id -> [0, num_classes)+[bg_label]
                matched_idxs, matched_labels, targets[b]['obj_labels']
            )
            pos_obj_pr_inds = sampled_idxs[sampled_gt_classes != self.bg_label]
            pos_obj_gt_inds = matched_idxs[pos_obj_pr_inds]
            pos_obj_pr_inds, pos_obj_gt_inds = self.postprocess_indices(pos_obj_pr_inds, pos_obj_gt_inds, obj_iou)

            pos_pr_inds = pos_obj_pr_inds.unsqueeze(0)
            pos_gt_inds = pos_obj_gt_inds.unsqueeze(0)
            iou = obj_iou

            if verb_iou.shape[0]>0:
                matched_idxs, matched_labels = self.proposal_matcher(verb_iou)  # proposal_id -> highest_iou_gt_id, proposal_id -> [1 if iou > 0.6, 0 ow]
                sampled_idxs, sampled_gt_classes = self._sample_proposals(  # list of sampled proposal_ids, sampled_id -> [0, num_classes)+[bg_label]
                    matched_idxs, matched_labels, targets[b]['verb_labels']
                )
                pos_verb_pr_inds = sampled_idxs
                pos_verb_gt_inds = matched_idxs[pos_verb_pr_inds]
                pos_verb_pr_inds, pos_verb_gt_inds = self.postprocess_indices(pos_verb_pr_inds, pos_verb_gt_inds, verb_iou)

                if pos_verb_pr_inds.shape[0]>0:
                    for i in range(pos_verb_pr_inds.shape[0]):
                        if pos_verb_pr_inds[i] not in pos_pr_inds:
                            pos_pr_inds = torch.cat((pos_pr_inds,pos_verb_pr_inds[i].reshape(1,1)),dim=-1)
                            pos_gt_inds = torch.cat((pos_gt_inds,pos_verb_gt_inds[i].reshape(1,1)),dim=-1)
            pos_pr_inds = pos_pr_inds.squeeze(0)
            pos_gt_inds = pos_gt_inds.squeeze(0)
...
class Stage1Assigner(nn.Module):
    def __init__(self, num_queries, t_low=0.3, t_high=0.7, max_k=4):
        super().__init__()
    ...
    def _subsample_labels(self, label):
        ...
        pos_idx, neg_idx = subsample_labels(
            label, self.num_queries, self.positive_fraction, 0
        )
    ...
    def forward(self, outputs, targets):
        ...
        for b in range(bs):
            if len(targets[b]['obj_boxes']) == 0:
                indices.append((torch.tensor([], dtype=torch.long, device=targets[b]['obj_boxes'].device),
                                torch.tensor([], dtype=torch.long, device=targets[b]['obj_boxes'].device)))
                continue
            ...
            # Subject
            sub_iou, _ = box_iou(
                  box_cxcywh_to_xyxy(targets[b]['sub_boxes']),
                  box_cxcywh_to_xyxy(outputs['pred_sub_boxes'][b]),
            )
            ...
            all_pr_inds = torch.arange(self.num_queries).to(iou.device)
            ...
            pos_pr_inds, pos_gt_inds = pos_pr_inds.to(iou.device), pos_gt_inds.to(iou.device)
            indices.append((pos_pr_inds, pos_gt_inds))
