...
class HorNet(nn.Module):
    def __init__(self, in_chans=3, num_classes=1000, 
        ...
        for i in range(4):
            ...
            self.norm.append(LayerNorm(dims[i], eps=1e-6, data_format="channels_first"))
            cur += depths[i]
...
def hornet_tiny_gf(pretrained=False,in_22k=False, **kwargs):
    ...
    **kwargs
    ...
    if pretrained is True:
        print('Pretrained weights: hornet_tiny_gf have been loaded')
        model.load_state_dict(torch.load('params/hornet_tiny_gf.pth')['model'], strict=False)
...
class BackboneBase(nn.Module):
    def __init__(self, backbone: nn.Module, train_backbone: bool):
        ...
        self.num_channels = backbone.dims
...
class Backbone(BackboneBase):
    ...
    def __init__(self, name: str,
                 pretrain: bool):
        if name == 'hornet_tiny_gf':
              backbone = hornet_tiny_gf(pretrained=pretrain)
