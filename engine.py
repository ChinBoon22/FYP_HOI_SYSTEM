...
from datasets.spfm_eval import SPFMEvaluator
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
                    ...
                    n_iter_to_acc: int = 1, print_freq: int = 100, sam=False, 
                    use_bg=False):
    """
    Training one epoch
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        # inference
        outputs = model(samples, targets=targets if use_bg else None)
        ...
        if (batch_idx + 1) % n_iter_to_acc == 0:
            if sam:
                optimizer.step(t=batch_idx, samples=samples, targets=targets, zero_grad=True)
            else:
                if max_norm > 0 and 'adan' not in optimizer.__class__.__name__.lower():
...
def evaluate(model, criterion, postprocessor, data_loader, device, epoch, args):
    ...
    elif args.dataset_file == "spfm":
        evaluator = SPFMEvaluator(preds, gts, args)
        stats = evaluator.evaluate()
