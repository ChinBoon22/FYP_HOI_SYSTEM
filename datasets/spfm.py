...
class SPFMDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, img_set, img_folder, anno_file, transforms, num_queries, num_inter_queries, args):
        ...
        self._valid_obj_ids = list(range(1, args.num_obj_classes + 1 )) # for spfm: [1,11]
        self._valid_verb_ids = list(range(1, args.num_verb_classes + 1)) # for spfm: [1,10]
    ...
    def __getitem__(self, idx):
        ...
        if self.img_set == 'train':
            # SPFM follows x_1, y_1, x_2, y_2 scheme
            boxes[:, 0::2].clamp_(min=0, max=w)
            boxes[:, 1::2].clamp_(min=0, max=h)
            # ensures that x_2 > x_1 and y_2 > y_1
            # in SPFM all annotations satisfy this
            ...
            for hoi in img_anno['hoi_annotation']:
                ...
                if sub_obj_pair in sub_obj_pairs: # if there are multiple interactions involving the same subject and object,
                    ...
                    sub_box_xyxy = torch.stack([(sub_box[0] - 0.5 * sub_box[2]), 
                                        (sub_box[1] - 0.5 * sub_box[3]),
                                        (sub_box[0] + 0.5 * sub_box[2]), 
                                        (sub_box[1] + 0.5 * sub_box[3])], dim=-1)
                    obj_box_xyxy = torch.stack([(obj_box[0] - 0.5 * obj_box[2]), 
                                        (obj_box[1] - 0.5 * obj_box[3]),
                                        (obj_box[0] + 0.5 * obj_box[2]), 
                                        (obj_box[1] + 0.5 * obj_box[3])], dim=-1)
                    verb_box = torch.Tensor([torch.min(sub_box_xyxy[0], obj_box_xyxy[0]), 
                              torch.min(sub_box_xyxy[1], obj_box_xyxy[1]),
                              torch.max(sub_box_xyxy[2], obj_box_xyxy[2]),
                              torch.max(sub_box_xyxy[3], obj_box_xyxy[3])])
                    verb_box = torch.Tensor([(verb_box[0] + verb_box[2]) / 2, 
                                          (verb_box[1] + verb_box[3]) / 2,
                                          (verb_box[2] - verb_box[0]), 
                                          (verb_box[3] - verb_box[1])])    

                    verb_labels.append(verb_label)
                    sub_boxes.append(sub_box)
                    obj_boxes.append(obj_box)
                    verb_vecs.append(verb_vec)
                    verb_boxes.append(verb_box)
            ...
            if len(sub_obj_pairs) == 0:
                ...
                target['verb_boxes'] = torch.zeros((0, 4), dtype=torch.float32)
                ...
                target['verb_boxes'] = torch.stack(verb_boxes)
...
def make_spfm_transforms(image_set):
    ...
    scales = [416, 448, 480, 512, 544, 576, 608, 640]
    ...
    if image_set == 'train':
        return T.Compose([
            ...
            T.RandomSelect(
                ...
                T.Compose([
                    T.RandomResize([100, 200, 300]),
                    T.RandomSizeCrop(96, 300),
                    T.RandomResize(scales, max_size=640),
                ])
            ),
            normalize,
...
def build(image_set, args):
    ...
    CORRECT_MAT_PATH = os.path.join(root, 'annotations', 'corre_spfm.npy')

    img_folder, anno_file = PATHS[image_set]
    dataset = SPFMDataset(image_set, img_folder, anno_file, transforms=make_spfm_transforms(image_set),
...
if __name__ == "__main__":
    class args():
        ...
        test_path = "/data/spfm/images/test_nano"
